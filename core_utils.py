    ...
    if config.channel_id_2star_verification_forum and config.add_double_star_to_vip_ids_txt:
        try:
            ...
            if double_star_forum:
                double_star_threads = []
                async for thread in double_star_forum.archived_threads():
                    double_star_threads.append(thread)
    ...
    if not server_data_exist or config.reset_server_data_frequently:
        ...
        try:
            ...
            while fetch_more:
                ...
                messages = []
                async for message in webhook_channel.history(**options):
...
async def send_status_header(bot):
    ...
    try:
        ...
        try:
            messages = []
            async for message in commands_channel.history(limit=10):
                messages.append(message)
